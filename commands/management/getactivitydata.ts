...
export async function run(interaction : Discord.CommandInteraction, client : Discord.Client, args : Arguements[]) {
    ...
    let data
    try {
        data = await functions.getLogs(id);
    } catch(e) {
    ...
    return await interaction.editReply(functions.embedMaker(interaction.user, "Activity Data", `This staff member (<@${id}>) has completed ${totalAmountOfLogs} logs\n\n**Discord**: ${data.discord}\n**Reported Applications**: ${data.reportedApps}\n**Subdomains**: ${data.subdomains}`));
}
...
export let data = new Builders.SlashCommandBuilder()
    ...
    .addStringOption(option => option.setName("id").setDescription("The staff id of the staff member that you wish to view the activity data for").setRequired(false))