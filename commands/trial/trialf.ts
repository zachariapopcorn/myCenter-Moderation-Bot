...
export async function run(interaction : Discord.CommandInteraction, client : Discord.Client, args : Arguements[]) {
    ...
    let message = await fs.readFile("./messages/trialf.txt", "utf8");
    try {
...
export let data = new Builders.SlashCommandBuilder()
    .setName("trialf")
    .setDescription("Kicks a user with a fail trial message")
    .addStringOption(option => option.setName("id").setDescription("The id of the person to fail").setRequired(true));